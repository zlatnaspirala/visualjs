...
ROOT.CREATE_BOX_CANNON = function(position , dim ){
	...
	var halfExtents = new CANNON.Vec3(100,100,10);
	var boxShape = new CANNON.Box(halfExtents);
	var boxGeometry = new THREE.BoxGeometry(halfExtents.x*2,halfExtents.y*2,halfExtents.z*2);
	var boxBody = new CANNON.Body({ mass: 50 });
	boxBody.addShape(boxShape);
...
ROOT.CREATE_BOX_CANNON_STATIC = function(position , dim , rot ){
	...
	if (typeof rot != 'undefined') { boxMesh.rotation = rot;}
	...
	var mass = 2, radius = 20;
	CANNON_OBJECTS.sphereShape = new CANNON.Sphere(radius);
	ROOT.sphereBody = new CANNON.Body({ mass: mass });
	...
	ROOT.ballBody = new CANNON.Body({ mass: 1 });
	...
	ROOT.ballGeometry = new THREE.SphereGeometry(ROOT.ballShape.radius, 32, 32 );
