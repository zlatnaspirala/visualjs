...
var IMPORT = {
			...
			JS_JSON : function( name_ , path_ ) {			
			  var ROOT = this;
				...
				ROOT.MESH_OBJ = function ( geometry, materials ) { createSceneSkin(     geometry, materials, 0, 0, 50, 105 , ROOT , name_ ) };
...
//end of test 				
			...
			var ROOT = this;
            ROOT.JS_BONES_ANIMATION_DONE = function(){console.log('finished')};			
	...
	/* 		  // add animation data to the animation handler
			...
			JS_SKELETAL : function (name , path) {
				...
				//ROOT.blendMesh.applyWeight( 'walk', 1 / 3 );
			//	ROOT.blendMesh.applyWeight( 'run', 1 / 3 );
				...
				// try js catch json 
				try {
				//ROOT.blendMesh.load( path, ROOT.start );
				}
				catch(e){
				}
			  ...
			  ROOT.AUTO_UPDATE = function(){
				...
				ROOT.blendMesh.update( stepSize );
				 ROOT.helper.update();
				 //ROOT.gui.update( ROOT.blendMesh.mixer.time );
