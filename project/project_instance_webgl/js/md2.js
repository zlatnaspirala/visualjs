...
//@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
 	...
 	var MD2_config = {
					...
					//baseUrl: "res/md2/test/",

					body: "ratamahatta2.md2",
					//body: "pistol_body.md2",
...
if ( isPlayer == true ) {
			...
			ROOT.earthPivot = new THREE.Object3D();
					...
					//ROOT.cameraControls.target.set( 0, 50, 0 ); no help s here
		...
		ROOT.onKeyDown = function( event ) {
				...
				switch( event.keyCode ) {
					...
					case 38: /*up*/
					case 87: /*W*/ 	ROOT.controls.moveForward = true; break;

					case 40: /*down*/
					case 83: /*S*/ 	 ROOT.controls.moveBackward = true; break;

					case 37: /*left*/
					case 65: /*A*/   ROOT.controls.moveLeft = true; break;

					case 39: /*right*/
					case 68: /*D*/    ROOT.controls.moveRight = true; break;

					case 67: /*C*/     ROOT.controls.crouch = true; break;
					case 32: /*space*/ ROOT.controls.jump = true; break;
					case 17: /*ctrl*/  ROOT.controls.attack = true; break;
		...
		ROOT.onKeyUp = function ( event ) {
				...
				switch( event.keyCode ) {
					...
					case 38: /*up*/
					case 87: /*W*/ ROOT.controls.moveForward = false; break;

					case 40: /*down*/
					case 83: /*S*/ 	 ROOT.controls.moveBackward = false; break;

					case 37: /*left*/
					case 65: /*A*/ 	 ROOT.controls.moveLeft = false; break;

					case 39: /*right*/
					case 68: /*D*/ 	  ROOT.controls.moveRight = false; break;

					case 67: /*C*/     ROOT.controls.crouch = false; break;
					case 32: /*space*/ ROOT.controls.jump = false; break;
					case 17: /*ctrl*/  ROOT.controls.attack = false; break;
 ...
 if (gui_ == true) {
		  ...
		  ROOT.playbackConfig = {
			...
			if (typeof around !== 'undefined') {
				...
				ROOT.INDIKATOR1 = new THREE.Mesh( sphere, new THREE.MeshBasicMaterial( { color: 0xff0040 } ) )  
				ROOT.spotlight.add(ROOT.INDIKATOR1 );				
				
				ROOT.INDIKATOR2 = new THREE.Mesh( sphere, new THREE.MeshBasicMaterial( { color: 0xff0040 } ) )  
...
//#########################################
			...
			function setupSkinsGUI( ROOT ) {
				...
				var generateCallback = function( index ) {
					...
					return function () { ROOT.character.setSkin( index ); };
			...
			function setupGUIAnimations( ROOT ) {
				...
				var generateCallback = function( animationClip ) {
					...
					return function () { ROOT.character.setAnimation( animationClip.name ); };
 ...
 */
 ...
 function POINT_COLLISION (PLAYER_ROOT , indexOfCollison)
 {
    ...
    ROOT.COLLIDE;
	ROOT.LAST_POSITION  = {x:0,y:0,z:0};
	ROOT.LAST_POSITIONS = [];
							...
							var intersects = ROOT.GLOBAL_RAY.intersectObjects( scene.children , true );
									...
									if ( intersects.length > 0) {                  				  
				                     	COLLIDE = intersects[ 0 ].object;
											try {
                                             if ( intersects.length > 1) {console.log("collision for " +  intersects[ 1 ].object ); }
