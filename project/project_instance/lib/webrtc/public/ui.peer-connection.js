...
rtcMultiConnection.onopen = function(e) {
    ...
    addNewMessage({
        ...
        userinfo: 'Data connection is opened',
        color: e.extra.color
    });
...
rtcMultiConnection.onmessage = function(e) {
    ...
    whoIsTyping.innerHTML = '';
	 ...
	 MAIN_PEER.REMOTE_DATA.NETWORK_VIEW(e.data);
	 
    addNewMessage({
        header: e.extra.username,
        //message: 'Text message from ' + e.extra.username + ':' + (rtcMultiConnection.autoTranslateText ? linkify(e.data) + ' ( ' + linkify(e.original) + ' )' : linkify(e.data)),
		message: 'Msg from ' + e.extra.username + ':' + e.data ,
        userinfo:  'NEW MSG' ,
        color: e.extra.color
    });
...
rtcMultiConnection.onstream = function(e) {
	...
	if (e.mediaElement.id == rtcMultiConnection.localStreamids[0]){  console.log(e.mediaElement.id + " LOCAL JE")   }
